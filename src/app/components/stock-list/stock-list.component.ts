...
@Component({
  ...
  imports: [CommonModule, FormsModule, InputTextModule, ButtonModule, TableModule, CardModule, SkeletonModule],
...
export class StockListComponent implements OnInit {
  ...
  async loadStockData() {
    this.loading = true;
    this.stockRows = [];
    this.productService.getProducts('admin').subscribe(async (products: Product[]) => {
      const rows: any[] = [];
      for (const product of products) {
        if (product.productType === 'variant' && product.id) {
          const variants = await this.productService.getVariantsForProduct(product.id);
          console.log('Variants for product', product.id, variants);
          for (const variant of variants) {
            rows.push({
              productName: product.title?.en || ' this is a product variant',
          ...
          rows.push({
            productName: product.title?.en || 'No title',
            variantName: '',
